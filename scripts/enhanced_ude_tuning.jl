...
"""
    enhanced_ude_tuning.jl

Enhanced UDE hyperparameter tuning with research-grade search space and evaluation.
- Extended hyperparameter search: width, weight decay, learning rate, solver tolerance
- Multi-stage optimization strategy
- Advanced evaluation metrics
- Statistical significance testing
- Research-grade robustness analysis

Expected time: 2-3 hours for full search
"""
...
function build_theta(width::Int)
    W = width
    θ = 0.01 .* randn(W + W)  # Smaller initial weights for stability
    return θ
end
...
function eval_scenario_enhanced(params, width::Int, reltol::Float64, sdf::DataFrame)
    try
...
function total_loss_enhanced(params, width::Int, reltol::Float64, scenarios)
    losses = Float64[]
    
    for (sid, sdf) in scenarios
        try
            metrics = eval_scenario_enhanced(params, width, reltol, sdf)
...
function optimize_enhanced(width::Int, λ::Float64, lr::Float64, reltol::Float64, seed::Int)
    Random.seed!(seed)
    θ_size = width + width
    ...
    function obj(vecp)
        try
            q = copy(vecp)
            constrain!(q)
            loss = total_loss_enhanced(q, width, reltol, train_sc)
    ...
    for (sid, sdf) in val_sc
        m = eval_scenario_enhanced(bestp, width, reltol, sdf)
...
function agg_metrics_enhanced(df::DataFrame)
    function ci(v)
...
println("🚀 Starting enhanced hyperparameter search...")
...
for w in widths, λ in λs, lr in lrs, rt in reltols
    ...
    for s in seeds
        ...
        p, met = optimize_enhanced(w, λ, lr, rt, s)
        m = agg_metrics_enhanced(met)
...
CSV.write(joinpath(@__DIR__, "..", "results", "enhanced_ude_tuning_results.csv"), results)
BSON.@save joinpath(@__DIR__, "..", "checkpoints", "enhanced_ude_best.bson") best_ckpt best_cfg best_metrics
...
if best_metrics !== nothing
    m = agg_metrics_enhanced(best_metrics)
...
println("✅ Enhanced UDE tuning complete!")
